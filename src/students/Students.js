...
export default function Students() {
    ...
    const loadStudents = async () => {
        const result = await axios.get("http://localhost:8080/students-courses-details");
        ...
        <div>
            ...
            <header className="bg-white shadow">
                <div className="mx-auto max-w-7xl py-6 px-4 sm:px-6 lg:px-8">
                    <h1 className="text-3xl font-bold tracking-tight text-gray-900 py-5">Students</h1>
                </div>
            </header>
            <div className='px-4 py-5 text-left'>
                <a href="/new_student" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">New Student</a>
            </div>
            <div className='py-5 px-5'>
                <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
                    <table className="w-full text-lg text-center text-blue-100 dark:text-blue-100">
                        <thead className="text-xxs text-white uppercase bg-blue-600 dark:text-white">
                            <tr>
                        ...
                        <tbody className="text-xxs">
                            {
                                students.map((student, index) => (
                                    <tr className="bg-white-500 border-b border-vlack-400 dark:hover:bg-gray-200">
                                        ...
                                        <td className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-black">{student.firstName}</td>
                                        <td className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-black">{student.lastName}</td>
                                        <td className="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-black">{student.courseCode}</td>
                                        <td>
                                            <Link className='text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800' to={`/view_student/${student.studentId}`}>View</Link>
                                            <Link className='text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700' to={`/update_student/${student.studentId}`}>Update</Link>
                                            <Link className='text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900' onClick={() => deleteStudent(student.studentId)}>Delete</Link>
                                        </td>
                                    </tr>
                                ))
                            }
